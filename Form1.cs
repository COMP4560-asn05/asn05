...
namespace asgn5v1
{
    ...
    public class Transformer : System.Windows.Forms.Form
    {
        ...
        const double SCALE_RATIO = 0.1d;
        const double SHEAR_RATIO = 0.1d;
        const double ROTATE_ANGLE = 0.05d;
        const double TRANSLATE_H = 75d;
        const double TRANSLATE_V = 35d;
        ...
        double[,] ctrans_init = new double[4, 4];  // store the initial ctrans 
        double[,] ref_points;

        string auto_rotate_axis = "z";
        ...
        protected override void OnPaint(PaintEventArgs pea)
        {
            ...
            if (gooddata)
            {
                ...
                // transform the origin to the centre of the screen


                grfx.TranslateTransform(Width * (float)HALF, Height * (float)HALF);
                //now draw the lines
        ...
        bool GetNewData()
        {
            ...
            double[] z_range = getRangeByColum(vertices, 2);

            double[,] tr_init = Mtranslate(-SIZE * HALF, -SIZE * HALF, -(z_range[1] - z_range[0]) * HALF);
            double[,] rotate_reflect = Mreflect("xz");
            double fs = this.Height * HALF / SIZE;
            double[,] scale_init = Mscale(fs, fs, fs);

            ref_points = new double[,] {
                { SIZE * HALF, SIZE * HALF, (z_range[1] - z_range[0]) * HALF, 1},
                {0, 0, 0, 1},
            };

            ctrans = Mmul(Mmul(tr_init, rotate_reflect), scale_init);
            ctrans_init = ctrans;
        ...
        private void toolBar1_ButtonClick(object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
        {
            ...
            {
                double[,] m_tr = Mtranslate(-TRANSLATE_H, 0, 0);
                ctrans = Mmul(ctrans, m_tr);
                Refresh();
            }
            ...
            {
                double[,] m_tr = Mtranslate(0, -TRANSLATE_V, 0);
                ctrans = Mmul(ctrans, m_tr);
                Refresh();
            }
            ...
            {
                double[,] points = Mmul(ref_points, ctrans);
                double[,] m_tr = Mtranslate(0, -points[1, 1], 0);
                double[,] m_sh = Msheer(-SHEAR_RATIO, 0);
                double[,] m_tb = Mtranslate(0,points[1, 1], 0);
                ctrans = Mmul(Mmul(Mmul(ctrans, m_tr), m_sh), m_tb);
                Refresh();
            }
        ...
        void rotateby1step(string axis)
        {
            double[,] points = Mmul(ref_points, ctrans);

            double[,] m_tr = Mtranslate(-points[0, 0], -points[0, 1], -points[0, 2]);
            double[,] m_r = Mrotate(axis, ROTATE_ANGLE);
            double[,] m_tb = Mtranslate(points[0, 0], points[0, 1], points[0, 2]);
            ctrans = Mmul(Mmul(Mmul(ctrans, m_tr), m_r), m_tb);
            Refresh();
        }
        ...
        void scale(double ratio)
        {
            double[,] points = Mmul(ref_points, ctrans);
            double[,] m_tr = Mtranslate(-points[0, 0], -points[0, 1], -points[0, 2]);
            double[,] m_sc = Mscale(ratio, ratio, ratio);
            double[,] m_tb = Mtranslate(points[0, 0], points[0, 1], points[0, 2]);
            ctrans = Mmul(Mmul(Mmul(ctrans, m_tr), m_sc), m_tb);
            Refresh();
        }

        double[,] Mmul(double[,] a, double[,] b)
        {
        ...
        double[,] Mtranslate(double x, double y, double z)
        {
        ...
        double[,] Msheer(double shx, double shy = 0)
        {
            double[,] matrix = new double[4, 4];
            setIdentity(matrix, 4, 4);
        ...
        double[,] Mrotate(string axis, double angle)
        {
            double[,] matrix = new double[4, 4];
            switch (axis)
            {
        ...
        double[,] Mscale(double sx, double sy, double sz)
        {
            double[,] matrix = new double[4, 4];
            matrix[0, 0] = sx;
            matrix[1, 1] = sy;
        ...
        double[] getRangeByColum(double[,] a, int k)
        {
